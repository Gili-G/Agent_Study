...
from structed_output_cn import (
    DiscussionModelCN,
    get_vote_model_cn,
    WitchActionModelCN,
    get_seer_model_cn,
...
class ThreeKingdomsWerewolfGame:
    def __init__(self):
        ...
        self.wolvesplayers: List[ReActAgent] = []
    ...
    # 创建游戏玩家
    async def create_players(self, role:str, character:str)->ReActAgent:
    ...
    async def setup_game(self, player_count: int = 6):
        ...
        # 1. 创建全部所需角色
        characters = random.sample([ "刘备", "关羽", "张飞", "诸葛亮", "赵云",
            "曹操", "司马懿", "周瑜", "孙权"],player_count)
            # 人数不同，所建立的玩家角色配置数量不一样
